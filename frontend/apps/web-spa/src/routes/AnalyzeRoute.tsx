...
export function AnalyzeRoute() {
  ...
  if (!cvId) return <div data-testid="view-job-match" className="mx-auto max-w-3xl space-y-5 p-8"><h1 className="text-2xl font-bold">Phân tích JD</h1><select aria-label="Chọn CV" value={selected} onChange={(e) => setSelected(e.target.value)} className="w-full rounded-xl border p-3"><option value="">Chọn CV</option>{cvs.map((cv) => <option key={cv.id} value={cv.id}>{cv.title}</option>)}</select><textarea aria-label="Mô tả công việc" value={jd} onChange={(e) => setJD(e.target.value)} rows={12} className="w-full rounded-xl border p-3" placeholder="Dán JD (ít nhất 50 ký tự)" /><button disabled={busy || !selected || jd.trim().length < 50} onClick={() => void submit()} className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">{busy ? 'Đang phân tích…' : 'Phân tích'}</button>{error && <p role="alert" className="text-sm text-rose-600">{error}</p>}</div>
