...
export function AnalyzeRoute() {
  ...
  useEffect(() => { if (!cvId) void listCVs().then(setCVs).catch(() => setError(t('cvListLoadFailed'))) }, [cvId])
  useEffect(() => {
    ...
    source.onerror = () => { if (busy) setError(t('analyzeProgressFailed')); source.close() }
    return () => source.close()
  }, [cvId])
  ...
  async function submit() {
    ...
    catch (err) { setBusy(false); setError(err instanceof ApiError ? err.message : t('analyzeStartFailed')) }
  }
  ...
  return <div data-testid="view-job-match" className="mx-auto max-w-5xl space-y-5 p-8"><h1 className="text-2xl font-bold">{t('matchResult')}</h1>{busy && !report && <p className="text-sm text-slate-500">{t('matching')}</p>}{report?.degraded && <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">Phân tích đang ở chế độ suy giảm: {report.degradedReason ?? t('someModelsUnavailable')}.</p>}{report && <><div className="rounded-2xl border bg-white p-5"><span className="text-4xl font-bold text-indigo-700">{report.overall ?? 0}%</span><p className="mt-2 text-sm text-slate-600">{t('overallMatch')}</p></div><div className="grid gap-3 sm:grid-cols-2">{Object.entries(report.breakdown ?? {}).map(([key, value]) => <div key={key} className="rounded-xl border bg-white p-4"><span className="text-xs uppercase text-slate-500">{key}</span><strong className="ml-2">{value}%</strong></div>)}</div><section className="rounded-2xl border bg-white p-5"><h2 className="font-bold">{t('gaps')}</h2><ul className="mt-3 space-y-2 text-sm">{(report.gaps ?? []).map((gap) => <li key={gap.id} className="rounded-lg bg-slate-50 p-3"><strong>{gap.requirement}</strong><span className="ml-2 text-slate-500">{gap.advice ?? gap.reason}</span>{gap.kbRefs?.length > 0 && citations.length > 0 && <div className="mt-2 border-l-2 border-indigo-200 pl-3 text-xs text-slate-600">{citations.filter((citation) => gap.kbRefs.includes(citation.chunkId)).map((citation) => <p key={citation.chunkId}>“{citation.excerpt ?? citation.text ?? ''}” — {citation.authorName}{citation.authorTitle ? `, ${citation.authorTitle}` : ''}</p>)}</div>}</li>)}</ul></section></>}{error && <p role="alert" className="text-sm text-rose-600">{error}</p>}</div>
}
