...
import { cvToProfile, type CV } from '@hr/schema'
import { Elegant, Minimal } from '@hr/templates'
...
export function createPrintHandler(backendURL: string): RequestHandler {
  return async (req, res) => {
    ...
    const templateProps = { profile, theme: cv.theme as never, layout: cv.layout as never, variant: variant as 'presentation' | 'ats' | 'thumbnail' }
    const template = cv.templateId === 'minimal' ? <Minimal {...templateProps} /> : <Elegant {...templateProps} />
    const title = escapeTitle(cv.title ?? 'CV')
    const html = `<!doctype html><html lang="${cv.language === 'en' ? 'en' : 'vi'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>${PRINT_CSS}</style></head><body>${renderToStaticMarkup(template)}</body></html>`
